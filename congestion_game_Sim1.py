...
class Congestion(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fc000 = nn.Linear(input_size, 2048)
        self.bn000 = nn.BatchNorm1d(2048)
    ...
    def forward(self, x):
        x = F.relu(self.bn000(self.fc000(x)))
...
def New_Initialize_action(L, N, K, factor=5):
    """
    This function build action Matrix each player how much budget is spend on path Pi
    :param N: (int) number of players
    :param P: (int) number of paths from source to targer
    :return: Xe (2D-array) size NxP
    """
    # Check if L divided in factor
    assert L % factor == 0, "L should divided in factor and get (int). Please change L or factor"
    sample = int(L / factor)
    sigma_1 = 5.0
    sigma_2 = 1.0
    # Sampling from different distribution
    x1 = np.random.rand(sample, N, K)
    x2 = 5.0 * np.random.rand(sample, N, K)
    x3 = 0.1 * np.random.rand(sample, N, K)
    x4 = sigma_1 * np.random.randn(sample, N, K)
    x5 = sigma_2 * np.random.randn(sample, N, K)
    Xn_k = np.concatenate([x1, x2, x3, x4, x5], axis=0)
    row_sums = np.sum(Xn_k, axis=2, keepdims=True)
    Xn_k /= row_sums
    return Xn_k
...
if Generate_Train:
    ...
    np.save("Numpy_array_save/A.npy", A)
    for i in range(2):
        nash_Xn_k = Initialize_action(L, N, K)
        Xe_nash = np.sum(nash_Xn_k, axis=1)
        C_k_nash = A + B * Xe_nash + C * (Xe_nash ** 2)
...
# Build Xtest
X_test = np.concatenate([C_k_nash, Xe_nash, Xe_nash ** 2,
                          C_k_0, Xe_0, Xe_0 ** 2,
                          C_k_1, Xe_1, Xe_1 ** 2], axis=-1)

if Normalize_flag:
    epsilon = 1e-8
    file_path_mean = os.path.join("Numpy_array_save", "X_train(Mean).npy")
    X_mean = np.load(file_path_mean)
    file_path_std = os.path.join("Numpy_array_save", "X_train(Std).npy")
...
file_path_weights = os.path.join("trains_record", "Congestion_game_with_Adam", "ABC_est(best).pth")
input_size = 9 * K
output_size = 3 * K
...
for t in range(T):
    ...
    grad_nash_const = calc_gradient(C_k_nash_const_expanded, nash_const_Xn_k, B, C, Xe_nash_const, Nash_flag=True, beta=beta) ### This NN
    ###################################################################################################################################################
