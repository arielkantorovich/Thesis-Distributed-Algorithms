...
def objective_global_cost(d, N=0):
    ...
    :return: cost - scaler
             ...
             B = dx1 Matrix
    """
    C = 0.5 # Add some constant Value
    q = np.random.rand(d, d)
    Q = q @ q.T + C # Generate Q symmetric and self positve
    B = np.random.rand(d, 1) + C
    return float(-0.5 * B.T @ np.linalg.inv(Q) @ B), Q, B
...
def multi_agent_gradient_descent(Qn, Bn, N, d, learning_rate, T):
    ...
    final_cost = np.mean(cost_Matrix, axis=0).squeeze()
...
Objective_results, Q, B = objective_global_cost(d, N)
...
for n in range(N_exper):
    ...
    Qn = Q + q.transpose(0, 2, 1) @ q  # Generate Q self positive around real Q
    Bn = B + np.random.rand(N, d, 1) # Generate Bn around B
