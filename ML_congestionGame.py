...
file_path_x = os.path.join("Numpy_array_save", "N=50_K=5_congestion_game", "X_train(k=4).npy")
file_path_y = os.path.join("Numpy_array_save", "N=50_K=5_congestion_game", "Y_train(k=4).npy")
...
model = Congestion(input_size, output_size).to(device)
criterion = nn.MSELoss()
optimizer = optim.Adam(model.parameters(), lr=lr_init, betas=(0.9, 0.99))
